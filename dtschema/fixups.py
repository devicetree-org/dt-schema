...
import dtschema
from dtschema.lib import _get_array_range, _is_int_schema, _is_string_schema
...
unit_types_array_re = re.compile(
    "-(kBps|bits|percent|bp|db|mhz|sec|ms|us|ns|ps|mm|nanoamp|(micro-)?ohms|micro(amp|watt)(-hours)?|milliwatt|(femto|pico)farads|(milli)?celsius|kelvin|k?pascal)$"
)
...
def fixup_interrupts(schema, path):
    ...
    if "properties" in schema:
        ...
        if (
            schema["properties"].keys() & {"interrupts", "interrupt-controller"}
            and "interrupt-parent" not in schema["properties"]
        ):
            schema["properties"]["interrupt-parent"] = True

        if (
            "interrupts" in schema["properties"]
            and "interrupts-extended" not in schema["properties"]
        ):
            schema["properties"]["interrupts-extended"] = copy.deepcopy(
                schema["properties"]["interrupts"]
            )

    if (
        "required" in schema
        and "interrupts" in schema["required"]
        and (len(path) == 0 or path[-1] != "oneOf")
    ):
    ...
    if "dependentRequired" in schema:
        if (
            "interrupts" in schema["dependentRequired"]
            and "interrupts-extended" not in schema["dependentRequired"]
        ):
            schema["dependentRequired"]["interrupts-extended"] = schema[
                "dependentRequired"
            ]["interrupts"]

        for prop, val in schema["dependentRequired"].items():
            ...
            schema["dependentRequired"][prop].remove("interrupts")
            if len(schema["dependentRequired"][prop]) == 0:
                schema["dependentRequired"].pop(prop)
                break
    ...
    if (
        ...
        and "interrupts" in schema["dependentSchemas"]
        and "interrupts-extended" not in schema["dependentSchemas"]
    ):
        schema["dependentSchemas"]["interrupts-extended"] = schema["dependentSchemas"][
            "interrupts"
        ]


known_variable_matrix_props = {"fsl,pins", "qcom,board-id"}
...
def fixup_node_props(schema):
    ...
    if (
        "additionalProperties" in schema and schema["additionalProperties"] is True
    ) or (
...
def add_select_schema(schema):
    ...
    if (
        "$nodename" in schema["properties"]
        and schema["properties"]["$nodename"] is not True
    ):
        schema["select"] = {
            "required": ["$nodename"],
            "properties": {
                "$nodename": convert_to_dict(schema["properties"]["$nodename"])
            },
        }
